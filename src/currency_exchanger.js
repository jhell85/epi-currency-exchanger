...
export class CurrencyExchange {
  ...
  convert() {
    let header = `<div class="row"><div class="col-md-6 offset-md-3 output"><h2 class="text-center">${this.inFront ? this.symbol : ""}${this.amount}${this.inFront ? '': this.symbol} ${this.name} is trading at</h2></div></div> `;
    let printList = [];
    ...
    for (const code in rates) {
      for (const key in currencyFormat)
        if (code === key) {
          let convertedAmount =
            Math.round(rates[code] * this.amount * 100) / 100;
          printList.push(
            `<div>${convertedAmount} ${currencyFormat[key]["name"]}</div>`
          );
        }
    }
    printList.shift();
